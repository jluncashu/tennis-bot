    ...
    <div className="p-8">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Everyone who has booked a court, most recently active first.</p>
        </div>
        <input
          type="text"
          value={search}
      ...
      <div className="mt-6 overflow-x-auto rounded-lg border border-slate-200 bg-white">
        {loadError && <p className="p-4 text-sm text-red-600">Couldn't load customers: {loadError}</p>}
        {!loadError && loading && <p className="p-4 text-sm text-slate-500">Loading…</p>}
        {!loadError && !loading && filtered.length === 0 && (
          <p className="p-4 text-sm text-slate-500">
            {contacts.length === 0 ? "No customers yet." : "No customers match your search."}
          </p>
        )}
              ...
              {filtered.map((c) => (
                <tr key={c.id} onClick={() => setSelected(c)} className="cursor-pointer hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-slate-600">
                    {c.lastBookingDate ? dayjs(c.lastBookingDate).format("MMM D, YYYY") : "—"}
                  </td>
