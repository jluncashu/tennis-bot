...
import { listRulesForCourt, addRule, editRule, removeRule } from "./price-rules.service";
import { createPriceRuleSchema, updatePriceRuleSchema } from "./price-rules.schema";
...
export async function createRuleController(req: Request, res: Response) {
  const parsed = createPriceRuleSchema.safeParse(req.body);
  if (!parsed.success) {
