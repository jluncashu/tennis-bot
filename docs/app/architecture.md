...
description: How the API server, the database, and external systems (the WhatsApp Cloud API) interact today.
tags: [architecture]
status: stable
...
- The Postgres connection (`packages/api/src/config/db.ts`) is live but
  unused — no feature reads or writes to it yet. See
